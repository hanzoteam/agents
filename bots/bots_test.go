	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.numCreatedBots > 0 {
				...
				mockAPI.On("GetProfileImage", mock.AnythingOfType("string")).Return([]byte("re-encoded"), nil).Maybe()
				mockAPI.On("KVSetWithOptions", mock.AnythingOfType("string"), mock.Anything, mock.Anything).Return(true, nil).Maybe()
			}
